...
- Patchy is slower than Photoshop, especially on large documents. Canvas compositing and image flattening are optimized for multicore execution, splitting large images (4 Mpx+) into strips rendered across CPU cores. Desktop builds include one automatic canvas presentation backend based on Qt Quick/RHI: Qt selects OpenGL, Vulkan, Metal, or Direct3D according to the platform and driver, while Patchy rejects software renderers and falls back to the CPU widget in the same binary. The optional Dawn/WebGPU document-compositing tier can be enabled with `-DPATCHY_ENABLE_GPU_CANVAS=ON` and `-DPATCHY_ENABLE_WEBGPU=ON`; document compositing, PSD output, and compatibility tests remain CPU-authoritative, and unsupported features retain atomic CPU fallback. See [GPU canvas presentation](docs/gpu-canvas.md).
