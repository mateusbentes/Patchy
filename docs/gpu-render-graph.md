...
When `PATCHY_ENABLE_WEBGPU=ON` finds the pinned Dawn package, `WebGpuRenderBackend` connects the existing real WebGPU compositor to this contract. It builds a full or dirty mip-0 tile plan, validates the graph's dependency order, and records the accepted pass count before invoking Dawn. Each planned tile receives its own clear, compute-composite, and texture-to-buffer readback sequence; the WGSL shader adds the tile origin before sampling layer and mask textures. On a content update, the previous frame is copied to the output and only intersecting tiles are recomputed and read back.
