...
- `PATCHY_RENDER_BACKEND=auto|cpu|opengl|vulkan|metal|d3d11|d3d12` selects the Qt Quick/RHI graphics preference; `PATCHY_GPU_CANVAS=auto|cpu` remains an alias. A hardware scene graph can still fall back to the CPU compositor when the document is outside the GPU capability matrix.
