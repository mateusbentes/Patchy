...
- `PATCHY_BUILD_WEBGPU_VALIDATION_TESTS=ON` adds the manual `patchy_webgpu_equivalence_tests` executable. It is not a CTest entry and does not change the deterministic environment. With Dawn and hardware available, it compares native frames against `Compositor::flatten_rgb8()`, validates 255/256/257 pixel tile boundaries, supported masks and Blend If, and checks incremental dirty-tile readback. Without Dawn or hardware it reports `[SKIP]` rather than making the standard suite machine-dependent.
